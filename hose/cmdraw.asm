; If you don't want to link everything together with a bootloader, then this is for you.
; compile with "nasm cmdraw.asm -o boot.bin"
; run with "qemu-system-x86_64 boot.bin"
; you may need to "sudo apt install qemu-system-x86" before running.
; if you don't have nasm, you may also need to "sudo apt install nasm" to compile the program.
; the filesystem is REALLY small and can only hold three 27-byte files with 4-byte filenames.
; 0x8400: keyboard buffer location
; 0x7D98: filesystem location
; 0x9000: programs are loaded here
; 0x8E00: stack location
; 0x9800: guaranteed 0 bytes for at least 2 kB
; 0x7BFF: keyboard buffer length
...
section .text
    ...
    memcpy2:
        ret
    getfilepointer: ; gives a pointer to the file named DI. CX is set to 1 if the file doesn't exist. If the file does exist, SI will be set to a pointer to the file and CX will be set to 0.
        mov si, _cmddata1
        push si
        add si, 128
        mov bx, si
    ...
    getfilepointer3:
        ...
        iret
    _msg1:
        db "Hose started.", 0x0D, 0x0A, 0
    _msg2:
        db "cmd> ", 0
    _msg3:
        db "No such command.", 0x0D, 0x0A, 0
times 408 - ($-$$) db 0
    _cmddata1:
        db 'h'
        db 'a'
